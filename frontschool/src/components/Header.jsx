...
const Header = () => {
  ...
  const [activeSection, setActiveSection] = useState('#HeroCarousel');
  ...
  const handleScroll = useCallback(() => {
    ...
    const sections = ['#HeroCarousel', '#AboutSection', '#programs', '#Features', '#ContactSection'];
    const sectionPositions = sections.map(section => {
      const element = document.querySelector(section);
      return element ? element.getBoundingClientRect().top + window.scrollY : 0;
    });
    
    let currentSection = '#HeroCarousel';
    for (let i = sections.length - 1; i >= 0; i--) {
      if (window.scrollY >= sectionPositions[i] - 100) {
  ...
  const navigationItems = useMemo(() => [
    { href: '#HeroCarousel', label: 'Accueil' },
  ...
  // Dynamic header classes based on scroll - REMOVED the hide-on-scroll logic
  const headerClasses = useMemo(() => {
    const baseClasses = "fixed w-full top-0 z-[100] transition-all duration-300 ease-out";
  ...
  const logoClasses = useMemo(() => {
    const baseClasses = "relative overflow-hidden rounded-2xl p-3.5 mr-5 transition-all duration-300 group-hover:scale-105 group-hover:rotate-2";
    const scrolledClasses = scrollPosition > 20
  ...
  return (
    <header className={headerClasses}>
        ...
        <div className="flex justify-between items-center py-4 lg:py-5">
          ...
          <div className="flex items-center group cursor-pointer select-none">
            <div className={logoClasses}>
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-white/20 via-transparent to-white/5 group-hover:from-white/30 group-hover:to-white/10 transition-all duration-300" />
              ...
              <BookOpen className="h-8 w-8 text-white relative z-10 group-hover:rotate-12 transition-transform duration-300" />
            </div>
          ...
          <nav className="hidden lg:flex items-center space-x-2">
            {navigationItems.map((item, index) => {
              ...
              return (
                <a
                  ...
                  href={item.href}
                  className={`
                    group relative px-5 lg:px-6 py-3 font-semibold transition-all duration-300 rounded-xl
                    ${isActive 
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-blue-50/40 via-indigo-50/20 to-blue-50/40 opacity-0 group-hover:opacity-100 rounded-xl transition-all duration-300 scale-95 group-hover:scale-100" />
            ...
            <div className="ml-6 relative">
              <button 
                ...
                className="group relative overflow-hidden bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 hover:from-blue-700 hover:via-blue-800 hover:to-indigo-800 text-white px-8 py-3.5 rounded-2xl font-bold transition-all duration-300 shadow-xl shadow-blue-600/25 hover:shadow-2xl hover:shadow-blue-600/35 hover:-translate-y-0.5 active:translate-y-0 active:shadow-lg"
              >
          ...
          <button 
            className={`
              lg:hidden relative p-3.5 rounded-2xl transition-all duration-300 transform hover:scale-105 active:scale-95
              ${isMenuOpen 
            ...
            <div className="relative z-10 space-y-2">
              {navigationItems.map((item, index) => {
                ...
                return (
                  <a 
                    ...
                    href={item.href}
                    className={`
                      group relative flex items-center justify-between px-6 py-4 font-semibold transition-all duration-300 rounded-2xl
                      ${isActive 
                  ...
                  >
                    ...
                    {/* Hover gradient */}
                    <div className="absolute inset-0 bg-gradient-to-r from-blue-50/50 to-indigo-50/30 opacity-0 group-hover:opacity-100 rounded-2xl transition-all duration-300" />
                    
                    {/* Active indicator for mobile */}
                    {isActive && (
                      <div className="absolute left-2 top-1/2 transform -translate-y-1/2 w-1.5 h-1.5 bg-blue-600 rounded-full" />
                    )}
                  </a>
                );
              })}
              ...
              <div className="pt-4 mt-4 border-t border-gradient-to-r from-transparent via-blue-200/50 to-transparent">
                <button 
                  ...
                  className="group relative w-full overflow-hidden bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 hover:from-blue-700 hover:via-blue-800 hover:to-indigo-800 text-white px-8 py-5 rounded-2xl font-bold transition-all duration-300 shadow-xl shadow-blue-600/25 hover:shadow-2xl hover:shadow-blue-600/40 transform hover:-translate-y-0.5"
                >
                  
                  {/* Animated background */}
                  <div className="absolute inset-0 bg-gradient-to-r from-white/10 via-white/20 to-white/10 -skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-700" />
      ...
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-blue-300/60 to-transparent">
        <div className="h-full bg-gradient-to-r from-blue-400 to-indigo-400 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-center" />
      </div>
    </header>
  );
};
